...
import pandas as pd
import time
from test_script import TradingBot

# Main trading function
def main_trading_loop():
    bot = TradingBot(symbol='XAUUSD', timeframe=mt5.TIMEFRAME_D1, num_bars=300)
    ...
    while True:
        ...
        if bot.has_active_trade():
            print("An active trade is already open. Waiting for 1 minutes...")
            time.sleep(3600)  # Sleep for 1 minutes
            continue
        ...
        if confidence < 0.80:
            ...
            time.sleep(3600)  # Sleep for 15 minutes
            continue
        print(f"Predicted action: {predictionA}")

        if predictionA == 1:  # Buy
            entry_price = df['close'].iloc[-1] # Use the ask price for a buy trade
            stop_loss = entry_price - 5.00
            take_profit = entry_price + 4.50
            lotSize = 0.01
            order_id = bot.place_buy_trade(entry_price, stop_loss, take_profit, lotSize)
            if order_id:
            ...
            take_profit = entry_price - 4.50
            lotSize = 0.01
            order_id = bot.place_sell_trade(entry_price, stop_loss, take_profit, lotSize)
            if order_id:
                print(f"Sell trade placed: Order ID = {order_id}")

        # If no trade was placed, wait for 15 minutes
        if order_id is None:
            print("No favorable trading condition. Waiting for 5 minutes...")
            time.sleep(3600)  # Sleep for 15 minutes
        ...
        if order_id:
            while True:
                ...
                time.sleep(900)  # Check every 60 seconds for price updates
